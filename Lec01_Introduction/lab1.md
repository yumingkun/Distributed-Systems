...
你们获取的Map/Reduce实现缺少部分代码。在你能编写自己第一个map/reduce函数对之前，你们需要完善顺序的实现。特别的，我们提供的代码缺少两个重要部分：将输出分割成map任务的函数和为reduce任务收集全部输入的函数。这些任务将分别会被common_map.go里面的doMap函数和common_reduce.go里面的doReduce函数执行。这些文件中的注解应该会帮你正确理解代
